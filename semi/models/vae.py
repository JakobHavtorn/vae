...
from semi.layers import GaussianSample, GaussianMerge, GumbelSoftmax
from semi.inference import log_gaussian, log_standard_gaussian
...
class GumbelAutoencoder(nn.Module):
    def __init__(self, dims, n_samples=100):
        ...
        self.encoder = MLP([x_dim, *h_dim])
        self.sampler = GumbelSoftmax(h_dim[-1], z_dim, n_samples)
        self.decoder = MLP([z_dim, *reversed(h_dim), x_dim], output_activation=F.sigmoid)
