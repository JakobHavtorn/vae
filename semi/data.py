...
def get_mnist(location="./", batch_size=64, labels_per_class=100, num_workers=2):
    import torchvision.transforms as transforms
    from functools import reduce
    from operator import __or__
    from torch.utils.data.sampler import SubsetRandomSampler
    from torchvision.datasets import MNIST
    from semi.utils import onehot
    ...
    labelled = torch.utils.data.DataLoader(mnist_train,
                                           batch_size=batch_size,
                                           num_workers=num_workers,
                                           pin_memory=cuda,
                                           sampler=get_sampler(mnist_train.targets.numpy(), labels_per_class))
    unlabelled = torch.utils.data.DataLoader(mnist_train,
                                             batch_size=batch_size,
                                             num_workers=num_workers,
                                             pin_memory=cuda,
                                             sampler=get_sampler(mnist_train.targets.numpy()))
    validation = torch.utils.data.DataLoader(mnist_valid,
                                             batch_size=batch_size,
                                             num_workers=num_workers,
                                             pin_memory=cuda,
                                             sampler=get_sampler(mnist_valid.targets.numpy()))
